...
describe("Library/Config", () => {
  ...
  describe("#constructor", () => {
    ...
    it("Default config", () => {
      ...
      assert.deepStrictEqual(config.tracesPerSecond, 0, "Wrong tracesPerSecond");
      assert.deepStrictEqual(
    ...
    it("microsoft.rate_limited without arg keeps default tracesPerSecond in InternalConfig", () => {
      vi.stubEnv("OTEL_TRACES_SAMPLER", "microsoft.rate_limited");

      const config = new InternalConfig();

      assert.strictEqual(config.tracesPerSecond, 0, "Wrong tracesPerSecond");
