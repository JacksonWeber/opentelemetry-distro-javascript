...
- Report a real value for the `\Process(??APP_WIN32_PROC??)\% Processor Time Normalized` performance counter. It shared `lastAppCpuUsage` / `lastHrtime` / `lastCpusProcess` with the standard process time counter, so whichever observable callback ran second measured a near-zero delta and always reported `0`. [#212](https://github.com/microsoft/opentelemetry-distro-javascript/pull/212)
